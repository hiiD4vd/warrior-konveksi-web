...
export default function ProductDetail({ product, onBack }) {
  ...
  // Triggered when the mouse enters the image container
  const handleMouseEnter = () => {
    setIsHovering(true);
  };

  // Triggered when the mouse leaves the image container
  const handleMouseLeave = () => {
    setIsHovering(false);
  };

  // High performance tracking using refs instead of React state for coordinates
  const handleMouseMove = (e) => {
    if (!cursorRef.current || !containerRef.current) return;
    
    // clientX and clientY are perfectly accurate for position: fixed elements
    const { clientX, clientY } = e;
    
    // We update the transform style directly mapping to the coordinate.
    // The inner UI element has its own -translate-x-1/2 -translate-y-1/2 to perfectly center it.
  ...
  return (
    <section className="min-h-screen bg-[#f4f4f0] flex flex-col items-center justify-center p-8">
      ...
      <div className="mb-8 text-center pt-8">
        <h1 className="text-4xl font-bold uppercase tracking-tight text-[#1c1c1c]">{product ? product.title : "Exclusive Series"}</h1>
      ...
      >
        <img 
          src={product ? product.imageB : "https://images.unsplash.com/photo-1620799140188-3b2a02fd9a77?q=80&w=1400"} 
          alt={product ? product.title : "Product Model"} 
          className="w-full h-full object-cover transition-transform duration-700 ease-out hover:scale-105"
        />

        {/* 
          Custom Cursor Element 
          It uses fixed positioning and pointer-events-none to prevent intercepting hover
          events, ensuring smooth tracking without flickering.
        */}
        <div 
          ref={cursorRef}
          ...
          style={{
            // Start it off-screen initially so it doesn't flash in the corner
